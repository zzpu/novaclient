...
class Manager(utils.HookableMixin):
    ...
    def _list(self, url, response_key, obj_class=None, body=None):
        #api初始化在__init__
        #在novaclient\v1_1\client.py初始化servers的时候传入self，所以这里的api就是对应版本的client,这里是v1_1,
        #client初始化在novaclient\v1_1\client.py中的__init__函数，调用_construct_http_client初始化
        # _construct_http_client实现在novaclient\client.py,根据传入的session是否为None构造不同的http客户端,如果不为None构造SessionClient，如果为None构造HTTPClient
        #SessionClient和HttpClient都在novaclient\client.py实现
